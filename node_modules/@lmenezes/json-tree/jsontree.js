...
var JSONTree = (function() { // eslint-disable-line no-unused-vars
  ...
  var _excluirElem = function(){
    var onClick_ = 'onclick="var _nProperty=this.parentElement.getElementsByClassName(\'jstProperty\')[0].innerText;swal({title:\'Do you really want to delete \'+_nProperty+\'?\', icon:\'warning\', buttons:[true,\'Yes\'] }).then(val=>{if(val){this.parentElement.remove()}});"';
    return '<span class="jstExcluir" ' + onClick_ + '></span>';
  }
  ...
  var _excluirItem = function(){
    var onClick___ = 'onclick="swal({title:\'Do you really want to delete this node (Not saving it yet) ?\', icon:\'warning\', buttons:[true,\'Yes\'] }).then(val=>{if(val){ var avo=this.parentElement.parentElement; this.parentElement.remove(); var com=($(avo.lastChild).children().eq(avo.lastChild.childElementCount-1)); if($(com).hasClass(\'jstComma\')){com.remove()}   }});"';
    return '<span class="jstExcluirItem" ' + onClick___ + '></span>';
  }
 
  
  var _addItem = function(){
    var onClick__ = `onclick="swal({title:\'Add your query\',text:\'Enter your JSON data here\', icon:\'warning\', content:{element:\'input\', attributes:{className:\'code swal-content__input\'}},buttons:[true,\'Yes\'] }).then(val=>{if(val){try{
      JSON.parse(k(val));
      var e = document.createElement(\'span\');e.innerText=k(val)+\',\';this.appendChild(e.firstChild);document.getElementById(\'fileTree\').innerHTML=JSONTree.create(JSON.parse(document.getElementById(\'fileTree\').innerText))}catch(err){
      swal({title:\'Error\', text:\'That doesn\\\'t look like a JSON\', icon:\'error\'})
    }}});setCode();"`;
    return '<span class="jstArrAdd" ' + onClick__ + '></span>';
  }
  ...
  var _open = function(sym, id) {
    if(sym=="["){
      return _element(sym, {id: 'opening_' + id, class: 'jstBracket'})+(_addItem())
    }
