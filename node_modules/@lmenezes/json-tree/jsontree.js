var JSONTree = (function() { // eslint-disable-line no-unused-vars
  ...
  var _excluirItem = function(){
    var onClick___ = 'onclick="swal({title:\'Do you really want to delete this node ?\', icon:\'warning\', buttons:[true,\'Yes\'] }).then(val=>{if(val){ var avo=this.parentElement.parentElement; this.parentElement.remove(); var com=($(avo.lastChild).children().eq(avo.lastChild.childElementCount-1)); if($(com).hasClass(\'jstComma\')){com.remove()}   }});"';
    return '<span class="jstExcluirItem" ' + onClick___ + '></span>';
  }
  var _addItem = function(){
    var onClick__ = 'onclick="swal({title:\'Add your query\',text:\'Enter your JSON data here\', icon:\'warning\', content:{element:\'input\', attributes:{className:\'swal-content__input code\'}},buttons:[true,\'Yes\'] }).then(val=>{if(val){try{JSON.parse(val);var e = document.createElement(\'span\');e.innerText=val+\',\';this.appendChild(e.firstChild);document.getElementById(\'fileTree\').innerHTML=JSONTree.create(JSON.parse(document.getElementById(\'fileTree\').innerText))}catch(err){alert(\':(\')}}});"';
